...
Editor finaliza → envia link Frame.io → cliente assiste no browser sem instalar nada → comenta com timecode exato → editor vê comentários dentro do Premiere → corrige → sobe nova versão → ciclo até aprovação → Client Portal registra → Nextcloud entrega arquivo definitivo → Automation Engine publica automaticamente.
...
| Portal | Client Portal | Status tempo real · aprovações · histórico | $0 |
| Entrega | Nextcloud | Link download · arquivo definitivo | $0 |
| Automação | Automation Engine | Notificações · publicação YouTube/Vimeo | $0 |
...
| Aprovação | Informal | Client Portal — data + nome |
| Publicação | Manual · 1-2h | Automation Engine — automático |
...
- **Cliente** — portal Client Portal, Frame.io, Nextcloud — qualquer browser, qualquer país

### Fluxo distribuído

Captação em qualquer lugar → VPN criptografada → hub central processa (transcrição + indexação + análise) → editor trabalha com proxies → render farm finaliza → Frame.io para review → Client Portal registra aprovação → Automation Engine publica.
...
**Inclui:** Hardware recomendação, estrutura de projetos, pipeline ingest, Whisper, proxies, Premiere integration, Frame.io, Client Portal, storage tiers, VPN, treinamento meio dia.
...
**Inclui:** Tudo do Nível 1 + servidor dedicado GPU, Semantic Index, descrição visual por frame, MusicGen, file monitor automático, tier manager, Nextcloud, Client Portal avançado, VPN completa, treinamento dia completo.
...
**Inclui:** Tudo dos Níveis 1+2 + render farm distribuído, publicação automática, ComfyUI, pipeline multi-país, dashboard monitoramento, Client Portal multi-tenant, integração móvel, treinamento 2 dias, SLA.
...
**SaaS on-site:** Entrada reduzida + assinatura mensal. Switching cost altíssimo — uma vez integrado com Client Portal, Frame.io e Automation Engine, o cliente nunca sai.
